...
const Login = () => {
  ...
  const [role, setRole] = useState('manager'); // default to manager/admin
  ...
  const loginForm = useFormik({
    ...
    onSubmit: (values) => {
      let endpoint = '';
      if (role === 'employee') {
        endpoint = '/employee/authenticate';
      } else {
        endpoint = '/user/authenticate/';
      }
      axios.post(`${process.env.NEXT_PUBLIC_API_URL}${endpoint}`, values)
        .then((res) => {
          toast.success('Login Successful');
          const data = res.data;
          if (role === 'employee') {
            localStorage.setItem('employee_token', data.token);
            localStorage.setItem('employee_id', data.employee?._id); // optional
      ...
      <div className="relative z-10 w-full max-w-md mx-auto">
        <div className="bg-white/90 dark:bg-neutral-900 rounded-xl shadow-lg p-8">
          <div className="text-center mb-6">
            <h1 className="text-3xl font-bold text-blue-700 dark:text-white mb-2">Login</h1>
          ...
          <div className="mb-4 flex justify-center gap-4">
            <label className="flex items-center gap-2">
              ...
              />
              <span className="text-sm">Manager/Admin</span>
            </label>
            <label className="flex items-center gap-2">
              <input
                type="radio"
